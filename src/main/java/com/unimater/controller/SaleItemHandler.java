...
public class SaleItemHandler implements HttpHandler {
        ...
        if (segments.length > 2) {  // Adjust the index based on your URI structure
            try {
                ...
                if (saleItem != null) {
                    ...
                    exchange.getResponseHeaders().add("Content-", "application/json");
    ...
    // Handle POST requests (create new saleItem )
    private void handlePost(HttpExchange exchange) throws IOException {
        SaleItem saleItem = parseRequestBody(exchange, SaleItem.class);
        if (saleItem != null && saleItem.getId() == 0) { // Assuming ID is 0 for new sale
            saleItemDAO.upsert(saleItem); // Insert the new sale
    ...
    // Handle PUT requests (update existing saleItem )
    private void handlePut(HttpExchange exchange) throws IOException {
        SaleItem saleItem= parseRequestBody(exchange, SaleItem.class);
        if (saleItem != null && saleItem.getId() > 0) { // Existing saleItem needs a valid ID
            saleItemDAO.upsert(saleItem); // Update the saleItem
    ...
    private void handleDelete(HttpExchange exchange) throws IOException {
        ...
        if (segments.length > 2) {
            try {
                int id = Integer.parseInt(segments[2]); // Parse the ID
                saleItemDAO.delete(id); // Delete the saleItem
